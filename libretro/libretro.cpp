...
int SNDPAUSE=0,SHIFTON=0;

void retro_audiocb(signed short int *sound_buffer,int sndbufsize){
   int x; 
   if(pauseg==0 && SNDPAUSE==0)for(x=0;x<sndbufsize*2;x+=2)audio_cb(sound_buffer[x],sound_buffer[x+1]);	
}
...
extern int cap32_disk_dir(char *FileName);
...
int pre_main(const char *argv)
{
   ...
   Only1Arg = (strcmp(ARGUV[0],"caprice32") == 0) ? 0 : 1;
...
//		Add_Option("4");
   ...
   cmain(PARAMCOUNT,( char **)xargv_cmd); 
...
void retro_set_environment(retro_environment_t cb)
{
   ...
   struct retro_variable variables[] = {
      {
         "caprice32_sdl_autoboot","Autoboot disk; ON|OFF",
      },
      { NULL, NULL },
   };
...
static void update_variables(void)
{
   ...
   var.key = "caprice32_sdl_autoboot";
   var.value = NULL;
   ...
   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
      ...
      if (strcmp(var.value, "ON") == 0)
        ...
        fprintf(stderr, "[libretro-test]: Autoboot: %s.\n",autorun?"ON":"OFF");
   }
...
void retro_get_system_info(struct retro_system_info *info)
{
	...
	info->library_name = "caprice32-sdl";
	info->library_version = "2.8.1";
...
void retro_init(void)
{
   ...
   sprintf(retro_system_data, "%s%ccaprice32\0",RETRO_DIR,slash);
